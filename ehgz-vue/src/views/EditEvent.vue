...
async function submitEvent() {
    ...
    try {
        const response = await axios.patch(`${url}/api/events/${eventId}` , formData, {
            headers: {
...
async function populateEditFields() {
    const data = await axios.get(`${url}/api/events/${router.currentRoute.value.params.id}`);
    eventName.value = data.data.name;
    eventDate.value = data.data.date.split('T')[0];
    eventTime.value = data.data.date.split('T')[1].split(':')[0] + ':' + data.data.date.split('T')[1].split(':')[1];
    eventVenue.value = data.data.venue;
    eventPrice.value = data.data.price;
    eventCategory.value = data.data.category;
    eventDescription.value = data.data.description;
    previewUrl.value = data.data.image;

    tags.value = data.data.tags_display.join(' ');
}
